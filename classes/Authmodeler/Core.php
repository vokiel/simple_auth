...
class Core extends \Hanariu\Model\Database
{
  ...
  {
    $model = empty($model) ? __CLASS__ : '\\Model\\' . ucwords($model);
    return new $model($id);
  }
  ...
  {
    static $instance;
    $model = empty($model) ? __CLASS__ : '\\Model\\' . ucfirst($model);
  ...
  public function save()
  {
    ...
    // Do an update
    if ($this->loaded()) {
      $result = count(\Hanariu\DB::update($this->table_name)->set($data_to_save)->where($this->primary_key, '=', $this->data[$this->primary_key])->execute());
      if ($result) {
    ...
    {
      $id = \Hanariu\DB::insert($this->table_name, array_keys($data_to_save))->values($data_to_save)->execute();
      if ($id) {
      ...
      if ($id AND !empty($this->hash_field)) {
        \Hanariu\DB::update($this->table_name)->set(array($this->hash_field => sha1($this->table_name . $id . $this->hash_suffix)))->where($this->primary_key, '=', $this->data[$this->primary_key])->execute();
      }
  ...
  public function load($value, $key = NULL)
  {
    ...
    $data = \Hanariu\DB::select($this->select)->from($this->table_name)->where($key, '=', $value)->execute();
  ...
  public function delete()
  {
    if (intval($this->data[$this->primary_key]) !== 0)
      return \Hanariu\DB::delete($this->table_name)->where($this->primary_key, '=', $this->data[$this->primary_key])->execute();
  ...
  public function __wakeup()
  {
    ...
    $this->_db = \Hanariu\Database::instance();
  }
