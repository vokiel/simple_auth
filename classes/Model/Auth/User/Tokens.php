...
class Tokens extends \Authmodeler\Core
{
    ...
    if ($id != NULL AND is_string($id)) {
      ...
      if ($this->loaded()) {
        if ($this->data['expires'] < $this->now) {
          $this->delete_expired();
          $this->clear_data();
          \Hanariu\Cookie::delete(\Hanariu::$config->load('simpleauth')->cookie_key);
        }
      } else {
        \Hanariu\Cookie::delete(\Hanariu::$config->load('simpleauth')->cookie_key);
      }
  ...
  public function save()
  {
    if ($this->data[$this->primary_key] == 0) {
      ...
      $this->data['user_agent'] = sha1(\Hanariu\Request::user_agent('browser'));
    }
  ...
  {
    return \Hanariu\DB::delete($this->table_name)->where('expires', '<=', $this->now)->execute();
  }
  ...
  public function delete_user_tokens($id = 0, $all = FALSE)
  {
    ...
    if ($all)
      return \Hanariu\DB::delete($this->table_name)->where('user_id', '=', $id)->execute();
    else
      return \Hanariu\DB::delete($this->table_name)->where('user_id', '=', $id)->where('user_agent', '=', sha1(\Hanariu\Request::user_agent('browser')))->execute();
  }
  ...
  protected function create_token()
  {
    while (TRUE) {
      ...
      $token = \Hanariu\Text::random('alnum', 32);

      // Make sure the token does not already exist
      if (count(\Hanariu\DB::select('id')->from($this->table_name)->where('token', '=', $token)->execute()) === 0) {
