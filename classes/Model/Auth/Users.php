...
class Users extends \Authmodeler\Core
{
  ...
  public function __construct($id = NULL)
  {
    ...
    $auth_config = \Hanariu::$config->load('simpleauth');
  ...
  {
    $data = \Hanariu\DB::select('*')->from($this->table_name)->where($this->unique_field, '=', $unique)->and_where($this->password_field, '=', $pass)->execute();
  ...
  {
    if (!empty($second)) {
      return count(\Hanariu\DB::select('id')->from($this->table_name)->where($this->unique_field, '=', $name)->or_where($this->second_field, '=', $second)->execute());
    } else {
      return count(\Hanariu\DB::select('id')->from($this->table_name)->where($this->unique_field, '=', $name)->execute());
    }
  }
  ...
  public function save()
  {
    if ($this->is_sha1($this->password_field)) {
      ...
      $this->$password_field = \Simpleauth\Core::instance()->hash($this->$password_field);
    }
    if (!$this->is_sha1($this->{$this->password_field})) {
      $this->{$this->password_field} = \Simpleauth\Core::instance()->hash($this->{$this->password_field});
    }
    return parent::save();
  }
